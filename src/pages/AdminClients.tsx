...
import { generateRegistrationLink, getRegistrationUrl } from '@/services/registrationLinks'
...
export default function AdminClients() {
  ...
  const handleUnassignCoach = async (clientId: string) => {
    if (!confirm('¿Estás seguro de que quieres remover la asignación de este cliente?')) {
      return
    }
  ...
  const handleGenerateLink = async () => {
    ...
    try {
      setError('')
      const link = await generateRegistrationLink(selectedCoachForLink, user.id)
      ...
      <div className="space-y-6">
          ...
          <Dialog open={isLinkDialogOpen} onOpenChange={setIsLinkDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Genera un enlace que asignará automáticamente nuevos clientes al coach seleccionado
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                ...
                {registrationLink && (
                  <div className="p-3 bg-muted rounded-md">
                    <p className="text-sm font-medium mb-2">Enlace generado (copiado al portapapeles):</p>
                    <p className="text-sm text-muted-foreground break-all">{registrationLink}</p>
                  </div>
                )}
              </div>
              <DialogFooter>
                ...
                <Button onClick={handleGenerateLink} disabled={loading || !selectedCoachForLink}>
        ...
        <Dialog open={isAssignDialogOpen} onOpenChange={setIsAssignDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>
                {selectedClient ? `Asignar Coach - ${selectedClient.name}` : 'Asignar Coach'}
              </DialogTitle>
              <DialogDescription>
            ...
            <DialogFooter>
              ...
              <Button onClick={handleAssignCoach} disabled={loading || !formData.coach_id}>
